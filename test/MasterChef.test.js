...
contract('MasterChef', ([alice, bob, carol, referrer, treasury, dev, fee, owner]) => {
    beforeEach(async () => {
        ...
        this.referral = await PonyReferral.new({ from: owner });
        this.chef = await MasterChef.new(this.pony.address, '100', '1000', { from: owner });

        await this.pony.transferOwnership(this.chef.address, { from: owner });
        await this.referral.updateOperator(this.chef.address, true, { from: owner });
        await this.chef.setPonyReferral(this.referral.address, { from: owner });
